...
installer_state_t g_installer = {
    ...
    .root_password = "root",
    .progress_percent = 0,
    .status_message = "Ready to install LinuxOSZero.",
...
void app_launch_installer(void) {
    ...
    installer_win = wm_create_window("Install LinuxOSZero v1.0", ICON_INSTALLER, 180, 90, 640, 440, installer_render, installer_on_event);
}
...
void installer_render(window_t *win, int cx, int cy, int cw, int ch) {
    ...
    switch (g_installer.current_step) {
        case STEP_WELCOME:
            font_draw_string(mx, my, "Welcome to LinuxOSZero Installation", g_theme.text_primary, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx, my + 24, "This wizard will install LinuxOSZero on your system or VM.", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));

            /* System check card */
            canvas_draw_card(mx, my + 60, mw, 160, COLOR_RGB(30, 41, 59), g_theme.card_border);
            font_draw_string(mx + 16, my + 76, "System Compatibility Checklist:", g_theme.accent_hover, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 16, my + 104, "[OK] VirtualBox VMMDev & VMSVGA Drivers detected", g_theme.success, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 16, my + 128, "[OK] 2048 MB System RAM verified", g_theme.success, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 16, my + 152, "[OK] Target Storage Drive available (>10 GB)", g_theme.success, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 16, my + 176, "[OK] x86_64 Long Mode & ACPI support ready", g_theme.success, COLOR_RGBA(0, 0, 0, 0));
        ...
        case STEP_DISK_SELECT:
            ...
            font_draw_string(mx + 50, my + 98, "Partitioning: Auto-MBR + ext4 root + 512MB EFI/Boot", g_theme.text_muted, COLOR_RGBA(0, 0, 0, 0));
        ...
        case STEP_USER_CONFIG:
            font_draw_string(mx, my, "Configure User & System", g_theme.text_primary, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx, my + 24, "Set hostname and administrative credentials.", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));
            ...
            font_draw_string(mx + 16, my + 200, "Timezone       : UTC (auto-sync via VirtualBox)", g_theme.text_muted, COLOR_RGBA(0, 0, 0, 0));
        ...
        case STEP_INSTALLING:
            font_draw_string(mx, my, "Installing LinuxOSZero...", g_theme.text_primary, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx, my + 24, "Copying root filesystem, kernel and VirtualBox drivers...", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));
        ...
        case STEP_COMPLETE:
            ...
            font_draw_string(mx, my + 24, "LinuxOSZero is now installed on your virtual drive.", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));

            canvas_draw_card(mx, my + 60, mw, 160, COLOR_RGB(30, 41, 59), g_theme.success);
            ...
            font_draw_string(mx + 64, my + 104, "- Installed Kernel: LinuxOSZero 6.x x86_64", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 64, my + 126, "- Bootloader: GRUB2 / Hybrid MBR & EFI", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 64, my + 148, "- VirtualBox Guest Additions: Configured & Active", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));
            font_draw_string(mx + 64, my + 170, "- Default User: user | Password: zero", g_theme.text_secondary, COLOR_RGBA(0, 0, 0, 0));
...
void installer_on_event(window_t *win, int ev_type, int p1, int p2) {
    (void)win;
    if (ev_type != 1) return; /* Click event */
    ...
    if (g_installer.current_step == STEP_WELCOME) {
        ...
        if (p1 >= (mx + mw - 260) && p1 < (mx + mw - 160) && p2 >= (ch - 50) && p2 < (ch - 18)) {
            ...
            strcpy(g_installer.status_message, "Creating GPT/MBR partitions on /dev/sda...");
        }
    } else if (g_installer.current_step == STEP_INSTALLING) {
        /* Advance installation progress on click or timer */
        if (g_installer.progress_percent < 30) {
            g_installer.progress_percent = 35;
            strcpy(g_installer.status_message, "Formatting ext4 root filesystem...");
        } else if (g_installer.progress_percent < 60) {
            g_installer.progress_percent = 65;
            strcpy(g_installer.status_message, "Copying LinuxOSZero base system files...");
        } else if (g_installer.progress_percent < 85) {
