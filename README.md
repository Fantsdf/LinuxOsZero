...
5. **Современный интерфейс**:
   - **GRUB2-меню загрузки** с выбором режима + **Loading-экран** с анимацией и прогрессом.
   - **Видеокарта видна в системе**: VMSVGA / std VGA / VMware SVGA с объёмом VRAM.
   - **Настройка пользователя**: имя, пароль, аватар (сохраняется).
   - **Рабочая Самка** (файловый менеджер): вид «список/сетка», сортировка, скрытые файлы.
   - **Красивый установщик** как настоящая ОС: ввод имени хоста, пользователя, пароля, sudo.
