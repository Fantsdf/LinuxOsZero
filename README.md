...
4. **Встроенные приложения**:
   ...
   - `ZeroControlPanel`: центр управления разрешением экрана, драйверами VirtualBox и темами.
...
> `./release.sh --publish`, либо через CI (workflow `ci/build-release.yml`).

### 3. Интерактивная веб-версия рабочего стола
Не обязательно запускать VirtualBox, чтобы увидеть интерфейс — проект включает
**живой браузерный симулятор ZeroDesktop** (boot-экран, рабочий стол, оконный
менеджер, интерактивный терминал, установщик, менеджер файлов, редактор и панель управления):
...
├── ci/
│   └── build-release.yml             # GitHub Actions CI для сборки ISO и релиза
├── docs/
...
└── dist/
    ├── LinuxOSZero-v1.0.0-x86_64.iso # Готовый загрузочный образ для VirtualBox
